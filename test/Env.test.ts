...
describe('Environment', () => {
  describe('arguments', () => {
    test('constructs with default env', () => {
      expect(new Env('timeld').envPaths.env).toBe('TIMELD');
    });

    test('constructs with no env', () => {
      expect(new Env('timeld', { env: false }).envPaths.env).toBeUndefined();
    });

    test('grabs a variable from process env', async () => {
      process.env.TIMELD_TEST = 'Test';
      const args = (await new Env('timeld').yargs()).parseSync();
  ...
  describe('directories', () => {
    ...
    beforeEach(() => {
      // noinspection JSCheckFunctionSignatures
      tmpDir = dirSync({ unsafeCleanup: true });
      env = new Env('timeld', { data: tmpDir.name });
    });
